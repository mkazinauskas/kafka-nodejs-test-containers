...
async function waitForSchemaRegistry(baseUrl) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      throw new Error(`Schema Registry healthcheck failed: ${response.status} ${response.statusText} - ${body}`);
    } catch (error) {
      if (attempt === MAX_ATTEMPTS) {
        throw new Error(`Schema Registry did not become ready after ${MAX_ATTEMPTS} attempts`, { cause: error });
      }
...
async function fetchJson(url, options = {}) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Schema Registry request failed: ${response.status} ${response.statusText} - ${details}`);
  }
...
export default async function testSchemaRegistry(baseUrl) {
  ...
  if (JSON.stringify(parsedSchema) !== JSON.stringify(SCHEMA)) {
    throw new Error("Schema fetched from registry does not match the registered schema");
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("40405")) {
      ...
      console.warn("Schema Registry permanent delete returned 404; assuming subject was already removed.");
    } else {
      throw error;
    }
  }
